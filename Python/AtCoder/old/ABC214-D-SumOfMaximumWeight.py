class UnionFind:
    ...
    def union(self, x, y, w):
        ...
        すなわち、xの根の親を、yの根とする。
        """
        self.ans += self.size(x)*self.size(y)*w
        # self._add_tree(x)
        # self._add_tree(y)
        x = self.root(x)
        y = self.root(y)
...
def main():
    ...
    for _ in range(n-1):
        ...
        path.append([w, u-1, v-1])
    solv(n, path)


def solv(n:int , path: list):
    """solv
    iからjに行くときの最大の辺の重さをDP[i][j]とする
    DP[i][k] = max(DP[i][j], DP[j][k])
    パスに登録されているものについて、DP[u][v] = w
